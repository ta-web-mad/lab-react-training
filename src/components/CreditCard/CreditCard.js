...
const CreditCard = (props) => {
  ...
  const hidePassword = (string) => {
    ...
    return (
      hiddenPassword.substring(0, 4) +
      ' ' +
      hiddenPassword.substring(4, 8) +
      ' ' +
      hiddenPassword.substring(8, 12) +
      ' ' +
      hiddenPassword.substring(12, 16)
    );
  };
    ...
    <div className="credit-card" style={divStyle}>
      ...
      <div className="client-info">
        <p>
          ...
          {expirationYear} <span className="bank-name"> {bank} </span>{' '}
        </p>
        <p>{owner}</p>
      </div>
    </div>
